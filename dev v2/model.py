...
def exponential_decay(initial_lr, epoch, decay_rate=0.96, decay_steps=100):
...
class MultiLayerNetwork:
    def __init__(self, layer_sizes, n, lambd): # fn - funcion activacion
        ...
        self.beta2 = 0.999
        self.epsilon = 1e-8
        self.lambd = lambd  # Parámetro de regularización L2
    ...
    def compute_cost_with_l2(self, predictions, targets):
        m = targets.shape[0]
        cross_entropy_cost = cross_entropy_loss(predictions, targets)
        L2_regularization_cost = (self.lambd / (2 * m)) * sum(cp.sum(cp.square(w)) for w in self.weights)
    ...
    def backward(self, targets, learning_rate, t):
        ...
        for i in reversed(range(len(self.weights))):
            ...
            delta = error
            delta_weights[i] = cp.dot(self.activations[i].T, delta) / m + (self.lambd / m) * self.weights[i]
    ...
    def train(self, inputs, targets, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            adaptative_lr = exponential_decay(learning_rate, epoch)
            predictions = self.forward(inputs)
            error = self.compute_cost_with_l2(predictions, targets)
            ...
            if epoch % 10 == 0:
                ...
                if (last_error - error) < 0.00005: # si el costo mejora menos que esto durante 40 epocas seguidas, nos salimos
                    num_equal += 1
